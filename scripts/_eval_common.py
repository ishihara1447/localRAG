...
# --------------------------------------------------------------------------
#
# 2026-08-05: 4スクリプトに散在していた UNKNOWN 正規表現をここへ一本化した。
#
# 直した欠陥（EVAL_PROTOCOL 付録B の KNOWN 2件。どちらも「甘い方向」＝
# ハルシネーションを棄権＝正解と数えてしまう向きで、最も危険な誤りだった）:
#
#   c04_bare_mitsukari: 裸の `見つかり` が「見つかりました」に一致していた
#   c05_bare_okotae   : 裸の `お答えでき` が「お答えできます」に一致していた
#
# 対処: 肯定形にも一致する語幹は、**否定の語尾を必ず伴う**形に変えた。
#   見つかり     → 見つかりません / 見つかりませんでした / 見つからな…
#   お答えでき   → お答えできません / お答えできかねます
#
# 「ありません」「記載がない」等、それ自体が否定を含む語はそのまま残している。
#
# 🔴 この判定を各スクリプトで再定義しないこと。3度目の同種事故を防ぐため
#    一本化した（`docs/RAG_EVAL_INTERNAL_AUDIT_2026-07-16.md` §1-1/§1-2 と同じ理由）。

UNKNOWN = re.compile(
    r"不明"
    r"|見つかりません|見つかりませんでした|見つからな"
    r"|ありません|情報がない"
    r"|含まれていない|含まれていません|含まれておりません"
    r"|記載がない|記載されていない|記載されていません"
    r"|わかりません|分かりません"
    r"|お答えできません|お答えできかねます|回答できません"
    r"|存在しません"
)
...
def is_unknown(text: str) -> bool:
    ...
    文書に無いことを問われたときに棄権できているかの判定に使う。
    ハルシネーションを棄権と数えないことが最優先なので、
    肯定形にも一致する語幹は否定語尾を必須にしてある。
    """
    return bool(UNKNOWN.search(strip_think(text or "")))
