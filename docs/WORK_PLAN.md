...
- [~] self-hosted 起動: `anything-llm/docker` のソースビルドは**コンテナ内 github.com DNS 解決失敗で不可**（`docs/ENVIRONMENT.md`）。暫定で**公式イメージ `runtime/docker-compose.yml` で起動**して `http://localhost:3001` 確認 ← 実行中
