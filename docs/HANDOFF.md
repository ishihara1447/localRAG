...
> - **【2026-07-24 完了】Windows native v1.2.3 配布パッケージ(インストーラ/アンインストーラ堅牢化＋PS5.1日本語エンコーディング修正版)ビルド済み・インストール可能**。成果物: **`C:\LocalRAG\dist\OTE-RAG-Setup.exe`(23,552 bytes)＋`OTE-RAG-win64-v1.2.3.zip`(11,018,845,286 bytes, SHA-256=`3f92c0d1724782db74fd2b5ccd1a3c2ca0c0260193563d9155c6d91c2e5550cb`)＋`.sha256`**(SHA整合検証OK)。旧v1.2.2以前は削除(Setup.exeは隣のv*.zipを1個だけglob要求)。中身のRAG改良はv1.2.1と同じ(P1/think:false/出典ページ番号/cushion/hybrid/リブランド全部入り)。**Setup.exeは未署名**(SmartScreen「詳細情報→実行」)。
