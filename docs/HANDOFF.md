...
> - **【2026-07-24 完了】Windows native v1.2.4 配布パッケージビルド済み・インストール可能**。成果物: **`C:\LocalRAG\dist\OTE-RAG-Setup.exe`(26,624 bytes)＋`OTE-RAG-win64-v1.2.4.zip`(11,018,845,293 bytes, SHA-256=`fa0e6c0538eddd57f17247cfb0523dbfb56fc5e7f860b126dbe523ab7d095a17`)＋`.sha256`**(SHA整合検証OK)。旧v1.2.3以前は削除(Setup.exeは隣のv*.zipを1個だけglob要求)。中身のRAG改良はv1.2.1と同じ(P1/think:false/出典ページ番号/cushion/hybrid/リブランド全部入り)。**Setup.exeは未署名**(SmartScreen「詳細情報→実行」)。
> - **v1.2.4の差分(ユーザー要望2件対応)**: (1)**旧バージョン検出→同意→自動アンインストール**: Setup.exeが起動時にARP登録または`InstallRoot\app`から旧版を検出し、「削除して続行しますか(文書データは保持されます)」と確認→OKなら旧`uninstall.ps1`をサイレント実行してからインストール継続(Cancel/自動削除不可時はbusyロック前なので画面は操作可能なまま)。(2)**アンインストーラのデスクトップショートカットを廃止**、`InstallRoot\Uninstall-OTE-RAG.cmd`直接実行と「設定→アプリ」(ARP)の2経路に統一(install.ps1の各メッセージ・INSTALL_GUIDE.mdを更新、旧版が作ったデスクトップアイコンの掃除はuninstall.ps1側に互換性維持で残置)。**Webブラウザからのサービス起動/停止UIは調査の結果2026-07-13から実装済み**(`server/endpoints/localServices.js`+`LocalServicesPanel`+`LOCAL_SERVICE_CONTROL=winsw`)と確認、新規実装は不要だった。検証: csc実コンパイル成功(exit0)、PS5.1/pwsh7双方でinstall.ps1/uninstall.ps1のParseFile成功。
