...
> - **【2026-07-23 完了】Windows native v1.2.1 配布パッケージをビルド済み・インストール可能**。手順: fork現ソースを`C:\LocalRAG\src`へ全同期(cushion/hybrid/P1/think:false/出典ページ番号/リブランド全部入り)→ビルド済みfrontend(pageNumber表示入り)を`server/public`へ配置→既存Windows node_modules+prisma windowsエンジンを温存(依存整合確認済み・LanceDB0.15一致・欠落0で`yarn install`不要)→`export-windows.ps1 -Version 1.2.1`実行。成果物: **`C:\LocalRAG\dist\OTE-RAG-Setup.exe`＋`OTE-RAG-win64-v1.2.1.zip`(11,018,837,753 bytes, SHA-256=`0aa03a2d7ba5e8111b1b626e9d4c164523d962ddc2aa68049e26d6298a548fe5`, 100,621ファイル)＋`.sha256`**。SHA整合検証OK。旧v1.2.0一式は削除(Setup.exeは隣のv*.zipを1個だけglob要求のため)。**Setup.exeは未署名**(SmartScreen警告は「詳細情報→実行」)。
> - **残り=ユーザーによるWindows実機インストール＋動作検証**: `OTE-RAG-Setup.exe`ダブルクリック(UAC)→SHA検証→展開→install.ps1→ブラウザ起動。検証項目: **白書PDF投入→質問→引用モーダルに出典p.表示**、think:false/P1の回答品質、サービス制御UI/GPU(Session0 CUDA)、Round2回帰。インストールログ=`C:\ProgramData\OTE-RAG\InstallerLogs\`。
>
