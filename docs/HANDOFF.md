# 引き継ぎメモ（セッション間ハンドオフ）
> **【次セッションはここから 2026-07-18続き】精度向上の原因分析2件が完了、P1〜P4の改善計画を合意・未着手**
...
> - **P1 拒否前の自動言い換え再検索**（効果:実証済み・リスク:小・最優先）: 検索が弱い(スコア低/0件)ときだけ、LLMに質問を公式語彙へ1回言い換えさせ再検索してから回答。`server/utils/chats/stream.js`の検索→拒否判定の間に挿入。曖昧評価スクリプトで唯一の失敗が回復し明確版15/15が崩れないことを検証。
...
>
> **次セッション最初の一手**: P1（自動言い換え再検索）の実装から。`stream.js`の検索結果評価部に、スコア/件数が閾値未満のときだけ言い換え→再検索を挟むロジックを追加し、`scripts/ambiguous-eval.py`で回復を検証するところから開始する。
