...
- **モデル変更あり（2026-07-14）**: LLMを `qwen3:8b` → **`gemma4:12b`**（非中国系・Apache 2.0）に切替済み。
  `export-windows.ps1` の `$BundleModels` は gemma4:12b + bge-m3 に更新済み。
  **再ビルド前にビルドマシンで `ollama pull gemma4:12b` を実行**しておくこと（同梱blob解析の対象。同梱Ollama v0.31.2で対応）。
  既定システムプロンプトも gemma4 向けに調整済み（`server/models/systemSettings.js`、STEP1のsrc同期に含まれる）。
  詳細と評価根拠: `docs/MODEL_SELECTION_NON_CHINESE_2026-07-14.md` / `docs/MODEL_CARDS.md`。
- 生成物: `C:\LocalRAG\dist\LocalRAG-win64-v1.2.0.zip`。**配布zipは約2.3GB増**（gemma4:12bはqwen3:8bより約2.3GB大きい）。
...
7. **LLMが gemma4:12b で動作**すること（`/api/ps` で gemma4:12b がGPUロード・VRAM解放も確認）。E2E11項目・backup/stop/start・uninstallは回帰PASS。
   ※ 実運用精度は dev で検証済み（gemma4+調整prompt+topN8で25/30・捏造ゼロ）。Windows実機では「動作・GPU・回帰」を確認すればよい。
