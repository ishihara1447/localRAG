...
使うときは、デスクトップに作成される **OTE-RAG のショートカット**を
ダブルクリックするか、ブラウザ（Microsoft Edge など）で次の URL を
直接開きます。
...
ください。）ショートカットは疎通確認を兼ねており、サービスが停止して
いる場合は日本語の案内画面を表示します。
...
1. デスクトップの「LocalRAG」アイコンから OTE-RAG を開きます（ログインは不要です）。
2. 対象の**ワークスペース**（文書をまとめる箱。案件別・分野別に
   作成できます）を開きます。
3. ワークスペース名の横のアップロードアイコン（書類マーク）を
   クリックし、文書ファイルをドラッグ＆ドロップまたは選択します。
4. アップロードした文書を選び、「Move to Workspace」→「Add to queue」→「Save and Embed」で
...
> **画面からのサービス制御**: メモリ節約などのため、ブラウザ画面の
> 右上にある状態表示（サービスの稼働状況を示すピル）をクリックすると、
> AI 実行エンジン（Ollama）と文書読み取り（Collector）を画面から
> 個別に停止・起動できます。画面本体（Server）は画面を提供している
> ため画面からは止められません（`stop.ps1` で全体を停止します）。
