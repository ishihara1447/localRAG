...
```text
OTE-RAG-Setup.exe
OTE-RAG-win64-v1.2.12.zip
OTE-RAG-win64-v1.2.12.zip.sha256
```
...
| GPU | NVIDIA 製 GPU、ビデオメモリ（VRAM）**8GB 以上**（16GB 推奨）。NVIDIA ドライバ導入済みであること。**8GB での動作は未検証です**（v1.2.12 時点） |
...
手動手順で使う配布物は `OTE-RAG-win64-v1.2.12.zip` です
（モデルを含むため数 GB あります）。展開すると以下の構成になります。

```
OTE-RAG-win64-v1.2.12\
...
tar.exe -xf .\OTE-RAG-win64-v1.2.12.zip
```
...
```powershell
cd C:\Users\<ユーザー名>\Downloads\OTE-RAG-win64-v1.2.12
...
OTE-RAG はいつでも削除できます。次のいずれかの方法でアンインストールしてください
（管理者の確認画面（UAC）が出たら「はい」を押します）。

- **方法A（かんたん・推奨）**: **「設定」→「アプリ」→「インストールされているアプリ」** を開き、
  一覧の **「OTE-RAG」** を選んで「アンインストール」を押します。
  （コントロールパネルの「プログラムと機能」からも同様に削除できます）
- **方法B**: インストール先フォルダ（既定 `C:\LocalRAG`）にある **`Uninstall-OTE-RAG.cmd`** を
  直接ダブルクリックします。

> インストール中に「新しいバージョンをインストールする前に、旧バージョンを自動で削除しますか？」
> と確認された場合は、上記の操作を手動で行う必要はありません（承諾すると自動で削除されます）。

アンインストール中に、次の確認が表示されます。

- **文書・データも完全に削除しますか？**
  - **いいえ（推奨）**: アプリだけを削除し、取り込んだ文書・検索データは
    `C:\ProgramData\LocalRAG\uninstalled-<日付>` に保管されます（後で復元・確認できます）。
  - **はい**: 文書・検索データ・AIモデルまで含めて完全に削除します（確認は二段階です）。

> 補足: インストール直後の完了メッセージにも、上記のアンインストール方法が表示されます。
