...
```text
OTE-RAG-Setup.exe
OTE-RAG-win64-v1.2.11.zip
OTE-RAG-win64-v1.2.11.zip.sha256
```
...
| GPU | NVIDIA 製 GPU、ビデオメモリ（VRAM）**8GB 以上**（16GB 推奨）。NVIDIA ドライバ導入済みであること。**8GB での動作は未検証です**（v1.2.11 時点） |
...
手動手順で使う配布物は `OTE-RAG-win64-v1.2.11.zip` です
（モデルを含むため数 GB あります）。展開すると以下の構成になります。

```
OTE-RAG-win64-v1.2.11\
...
tar.exe -xf .\OTE-RAG-win64-v1.2.11.zip
```
...
```powershell
cd C:\Users\<ユーザー名>\Downloads\OTE-RAG-win64-v1.2.11
