...
通常のインストールでは、zip の展開や PowerShell の操作は不要です。
提供元から受け取った次の 3 ファイルを、同じフォルダに置いてください。
...
2. ユーザーアカウント制御（UAC）が表示されたら「はい」を選びます。
3. インストール先と画面ポートを確認します。通常は変更不要です。
4. 「インストール」を押し、完了画面が出るまで待ちます。
5. 完了後、OTE-RAG がブラウザで自動的に開きます。

Setup.exe は、外側 zip の SHA-256 検証、展開、アプリケーションの
チェックサム検証、Windows サービス登録、起動確認を自動で実行します。
モデルを含むため、20〜60 分程度かかる場合があります。処理中は
パソコンを終了しないでください。

インストーラーログは
`C:\ProgramData\OTE-RAG\InstallerLogs` に保存されます。

> 現時点の Setup.exe はコード署名証明書を未取得のため、Windows に
> 「不明な発行元」と表示される場合があります。IT 担当者が配布元と
> 3 ファイルの組み合わせを確認したうえで実行してください。
...
手動手順で使う配布物は `OTE-RAG-win64-v1.2.0.zip` です
（モデルを含むため数 GB あります）。展開すると以下の構成になります。

```
OTE-RAG-win64-v1.2.0\
...
tar.exe -xf .\OTE-RAG-win64-v1.2.0.zip
```
...
```powershell
cd C:\Users\<ユーザー名>\Downloads\OTE-RAG-win64-v1.2.0
