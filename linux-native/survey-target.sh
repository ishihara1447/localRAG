...
# ---------------------------------------------------------------
sec "5. CPU 命令セット（AVX2）★v1.1.2 で新たに必要になりました"
...
if [ -r /proc/cpuinfo ]; then
  ...
  if grep -qm1 '^flags.*\bavx2\b' /proc/cpuinfo; then
    ...
    ng "  エラーにならず、静かに遅くなる・精度が落ちる形で出る可能性があります"
    ng "  （実際にどうなるかは未検証です）。"
    ng "  この結果を保守担当へお送りください。対応を検討します"
  fi
else
