...
rem --- パート数を数える ---------------------------------------------
set /a COUNT=0
for %%F in ("%~dp0%ZIPNAME%.0??") do set /a COUNT+=1
echo   見つかったパート: %COUNT% 個
echo.

rem --- 結合リストを作る（part00 から順に）---------------------------
set "LIST="
for /f "delims=" %%F in ('dir /b /on "%~dp0%ZIPNAME%.0??"') do (
...
echo.
rem Read the expected hash from the .sha256 sidecar, not MANIFEST.txt.
rem MANIFEST.txt is UTF-8 while this file is CP932, so findstr on the
rem Japanese label never matches. The sidecar is pure ASCII.
rem It ships anyway because OTE-RAG-Setup.exe requires it.
for /f "tokens=1" %%H in ('type "%~dp0%ZIPNAME%.sha256"') do (
    if not defined EXPECTED set "EXPECTED=%%H"
)

for /f "tokens=1" %%H in ('certutil -hashfile "%~dp0%ZIPNAME%" SHA256 ^| findstr /v ":"') do (
...
if /i "%ACTUAL%"=="%EXPECTED%" (
    ...
    rem ZIPNAME が空のまま del すると "<フォルダ>\*" になり、
    rem ダウンロードフォルダ全体の削除確認が出る。必ずガードする。
    if defined ZIPNAME del "%~dp0%ZIPNAME%" 2>nul
)
