...
2. 【重要】分割ファイル（.001 〜）を削除してください。
   ★これを先に行わないと、次のインストールが容量不足で失敗します。
   （インストール中は「結合したZIP」「一時展開」「インストール先」が
     同時に存在するため、分割ファイルを残すと約8GB 足りなくなります）

3. OTE-RAG-Setup.exe をダブルクリックしてください。
   管理者権限を求められたら「はい」を選んでください。
   「WindowsによってPCが保護されました」と出たら
   「詳細情報」→「実行」を押してください（署名を付けていないため出ます）。

必要な空き容量: 開始時に 40GB 以上
  （手順2で分割ファイルを消した場合。残したままなら 48GB 以上必要です）
  インストール後に製品が使う容量は約 9.9GB です。

インストール先を C: 以外にする場合でも、C: に約 22GB の空きが必要です
  （一時展開とモデルの置き場が C: 固定のため）。
